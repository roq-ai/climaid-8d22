...
function CompanyListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CompanyInterface[]>(
    ...
    () =>
      getCompanies({
        relations: ['user', 'environmental_analyst.count', 'team_member.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('environmental_analyst', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>environmental_analyst</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('environmental_analyst', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.environmental_analyst}</Td>
                    )}
                    {hasAccess('team_member', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (

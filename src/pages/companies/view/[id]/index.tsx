...
import { deleteEnvironmentalAnalystById, createEnvironmentalAnalyst } from 'apiSdk/environmental-analysts';
...
function CompanyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CompanyInterface>(
    ...
    () =>
      getCompanyById(id, {
        relations: ['user', 'environmental_analyst', 'team_member'],
      }),
  );

  const [environmental_analystUserId, setEnvironmental_analystUserId] = useState(null);
  const environmental_analystHandleCreate = async () => {
    setCreateError(null);
    try {
      await createEnvironmentalAnalyst({ company_id: id, user_id: environmental_analystUserId });
      setEnvironmental_analystUserId(null);
      await mutate();
    } catch (error) {
      setCreateError(error);
    }
  };
  const environmental_analystHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteEnvironmentalAnalystById(id);
      await mutate();
    } catch (error) {
            ...
            <Box>
              <Stack spacing={2} mb={8}>
                ...
                <Flex gap={5} alignItems="flex-end">
                  <Box flex={1}>
                    <UserSelect
                      name={'environmental_analyst_user'}
                      value={environmental_analystUserId}
                      handleChange={setEnvironmental_analystUserId}
                    />
                  </Box>
                  <Button
                    colorScheme="blue"
                    mt="4"
                    mr="4"
                    onClick={environmental_analystHandleCreate}
                    isDisabled={!environmental_analystUserId}
                  >
                    Create
                  </Button>
                </Flex>
                <TableContainer>
                  <Table variant="simple">
                    ...
                    <Tbody>
                      {data?.environmental_analyst?.map((record) => (
                        <Tr
                          cursor="pointer"
                          ...
                          <Td>
                            {hasAccess('user', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                              <IconButton
                                onClick={(e) => {
                                  e.stopPropagation();
                                  environmental_analystHandleDelete(record.id);
                                }}
                                colorScheme="red"
                                variant="outline"
                                aria-label="edit"

...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.environmental_analyst?.length > 0) {
      const create_environmental_analyst = body.environmental_analyst;
      body.environmental_analyst = {
        create: create_environmental_analyst,
      };
    } else {
      delete body.environmental_analyst;
    }
